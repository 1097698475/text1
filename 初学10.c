...
//	p22 = &p2;
//	pp = &p;
//	**pp = **p11;//**pp含义，pp为p的地址，*pp表示p中的内容，而p为c的地址，所以**pp为解引用pp得到p再解引用p
//	**p11 = **p22;
//	**p22 = **pp;
//	printf("%d %d", a, b);
//	return 0;
...
//指针简化表达
//数组a[10]
//p=a+strlen(a) 等价于p=&a[0+strlen(a)]
//
//解引用a
//n为某数
//但
...
//	if ((n <= 12) && (n >= 1))
//	{
//		printf("%s\n", *(month_name + n));//等价于*month_name[0+n]
//	}
//	else
//		printf("输入错误\n");
//	return 0;
...
//	gets(a);
//	//指针q指向字符尾串
//	q = &a[0 + strlen(a) - 1];//等价于q=a+strlen(a)-1
//	while (p < q)
//	{
//		if (*p != *q)
//		{
