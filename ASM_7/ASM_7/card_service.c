...
Card *askyCard(const char name[],const char pwd[])
{
	...
	p=head;
	while(p!=NULL){
		if(strcmp(p->cardData.aName,name)==0&&strcmp(p->cardData.aPwd,pwd)==0)
		{
			pCard=&p->cardData;
			break;
		}
...
int Recharge(const char* pName,const char* pPwd)
{
	...
	while(cardNode != NULL)
	{
		...
		if(strcmp(cardNode->cardData.aName,pName) == 0 && strcmp(cardNode->cardData.aPwd,pPwd) == 0&&(cardNode->cardData.nStatus !=2)&&(cardNode->cardData.nStatus !=4))
		{
			printf("请输入充值的金额：");
			scanf("%f",&charge);
			if(charge<100)
			...
			{
				cardNode->cardData.fBalance += charge+10;
			}
			if(charge>=300&&charge<500)
			{
				cardNode->cardData.fBalance += charge+50;
			}
			if(charge>=500)
			{
				cardNode->cardData.fBalance += charge+150;
			}
...
int Refund(const char* pName,const char* pPwd)
{
	...
	while(cardNode != NULL)
	{
		if(strcmp(cardNode->cardData.aName,pName) == 0 && strcmp(cardNode->cardData.aPwd,pPwd) == 0&&(cardNode->cardData.nStatus !=4))
		{
...
{
	if(strcmp(aName,"lsf")==0&&strcmp(aPwd,"1231")==0)
	{
		return TRUE;
	}
	return FALSE;
...
int doFind(const char aName[],char aId[])
{
	...
	if(strcmp(pCard->aId,aId)==0)
	{
		return TRUE;
	}
	return FALSE;
}
Card *queryId(const char id[])
{
